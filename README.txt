...
Добавлена лямбда validate_url. Возможно, стоит перехватывать исключения для случаев, когда пользователь вводит не существующий или не доступный URL. 
Как например, http://test.test
================================================================================    
14.10.2014 * AK

Добавил функцию count_indegree. Бета версия решения готова. Нужно отрефакторить код и проверить на возможнае баги. 

================================================================================   
17.10.2014 * IlyaKhlyzov

Под впечатлением последних skype-бесед добавил файл ss.rb пару дней назад. Обновил до состояния работает. (Возможно даже правильно). С регуляркой наладил общий язык. 
Предлагаю AK выложить его вариант в отдельный файл.
Функции в файле:
чтение_урл
генерирование_функции_парсить_ссылки
генерирование_функции_создать_абсолютную ссылку
список_ссылок
список_вхождений
сортировка_списка_вхождений

На следующей скайп-митинг обсудим текущие и дальнейшие планы.

================================================================================   
20.10 * IlyaKhlyzov
ss.rb:
- переписал блоки как do .. end

Задачи
- 1) Вылечить от `open_loop': redirection forbidden: http://translate.google.com/ -> https://translate.google.com/ (RuntimeError)
- 2) Разобраться с регулярным выражением! хавает такое:
если заданный url = Http://site.ru, то преобразоыввает //smth.site.ru
 в http://site//smth

================================================================================   
21.10 * IlyaKhlyzov

Замечание от EugeneHlyzov по ss.rb:
неправильное использование each (map, reduce)
не следование стайл-гайду

Старые задачи:
1) вылечить от редиректа
2) исцелить от http://site.ru: http://domen.site.ru -> //domen

Обновление задач:
0) Написать тесты (для класса)
1) Переписать задачу, используя классы
2) Сайт передавать как параметр (./ss.rb site)
3) Сохранять мета-теги
================================================================================   
22.10 * AK

После обсуждения с Ильёй пришли к выводу, что у нас будет один класс, а не несколько. 
У класса будет свойство URl, а все лямбды станут методами этого Класса. Также у класса будут
дополнительные свойства, в которых будут храниться все промежуточные результаты. Например,
...
Добавил "свою" версию решения. С классом и методами. Работает еще не так, как мне
хотелось бы, но возможно уже будет проще написать тесты.
